...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self = [super initWithFrame:frame]) {
    ...
    if (&MACaptionAppearanceCopyForegroundColor) {
      ...
      } else if (style.edgeStyle != kMACaptionAppearanceTextEdgeStyleNone &&
                 style.edgeStyle != kMACaptionAppearanceTextEdgeStyleUndefined) {
...
- (void)setFont:(NSString *)fontName frame:(CGRect)frame baseFontSize:(CGFloat)fontSize {
  ...
  CGFloat portraitWidth = mainBoundsSize.width > mainBoundsSize.height ?
                          mainBoundsSize.height : mainBoundsSize.width;
  CGFloat scalingFactor = frame.size.width / portraitWidth;
  self.font = [UIFont fontWithName:fontName size:fontSize * scalingFactor*arbitraryScalingFactor];
}
...
- (NSArray *)getRectsForEachLine:(NSArray *)separatedLines {
  ...
  for (NSString *line in separatedLines) {
    ...
    UITextPosition *start = [self positionFromPosition:beginning
    ...
    CGRect newTextRect = CGRectMake(textRect.origin.x,
                                    textRect.origin.y + 1.5,
                                    textRect.size.width,
